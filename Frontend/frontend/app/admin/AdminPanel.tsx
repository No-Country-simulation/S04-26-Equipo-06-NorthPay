...
import ContractorsTable, { Contractor } from "@/components/admin/ContractorsTable";
...
export default function AdminPanel() {
  ...
  const fetchContractors = async () => {
    try {
      ...
      if (res.ok) {
        const data = await res.json();
        const parseUtcDate = (dateStr: string | number | undefined | null) => {
        ...
        const mapped = data.content.map((item: any) => ({
          ...
          stage: `Step ${Math.min(item.currentStep || 1, 5)}`,
          status: item.status || "PENDING",
          startDate: parseUtcDate(item.createdAt).toLocaleDateString(),
          lastUpdate: parseUtcDate(item.updatedAt || item.createdAt).toLocaleString(),
          rawDate: parseUtcDate(item.createdAt).getTime(),
        }));
    ...
    <div className="min-h-screen bg-slate-50 py-12 px-6 sm:px-10">
      <div className="mx-auto max-w-6xl space-y-6">
        ...
        <ContractorsTable
          ...
          paginatedContractors={paginatedContractors}
          isLoading={isLoading}
